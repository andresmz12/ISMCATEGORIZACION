const en = {
  'app.name': 'My Profit and Loss',
  'app.short': 'My Profit & Loss',
  'app.tagline': 'Organize your expenses. Simplify your taxes.',

  // Nav
  'nav.dashboard': 'Dashboard',
  'nav.transactions': 'Transactions',
  'nav.import': 'Import',
  'nav.reports': 'Reports',
  'nav.rules': 'Rules',
  'nav.settings': 'Settings',
  'nav.admin': 'Administration',
  'nav.accounts': 'Accounts',
  'nav.plans': 'Plans',
  'nav.defaultCats': 'Default Categories',
  'nav.logs': 'Logs',
  'nav.team': 'Team',
  'nav.classify': 'Classify with AI',

  // Auth
  'auth.signin': 'Sign In',
  'auth.signup': 'Create Account',
  'auth.email': 'Email address',
  'auth.password': 'Password',
  'auth.confirmPassword': 'Confirm password',
  'auth.name': 'Full name',
  'auth.firmName': 'Firm name',
  'auth.loading': 'Loading...',
  'auth.invalid': 'Invalid email or password',
  'auth.noAccount': "Don't have an account?",
  'auth.hasAccount': 'Already have an account?',
  'auth.register': 'Sign up',
  'auth.signIn': 'Sign in',
  'auth.signout': 'Sign out',
  'auth.passwordMismatch': 'Passwords do not match',
  'auth.passwordShort': 'Min 8 characters, one uppercase and one number',

  // Register
  'register.step1.title': 'How will you use MyP&L?',
  'register.step1.subtitle': 'Choose the account type that best fits you',
  'register.accountant.label': "I'm an accountant / bookkeeper",
  'register.accountant.desc': 'Manage multiple clients and businesses from one place',
  'register.step2.title': 'Your account information',
  'register.step3a.title': 'Set up your business',
  'register.step3b.title': 'Set up your firm',
  'register.specialty': 'Specialty',
  'register.success': 'Account created! Sign in.',

  // Dashboard
  'dashboard.greeting.morning': 'Good morning',
  'dashboard.greeting.afternoon': 'Good afternoon',
  'dashboard.greeting.evening': 'Good evening',
  'dashboard.income': 'Income',
  'dashboard.expenses': 'Expenses',
  'dashboard.profit': 'Net Profit',
  'dashboard.deductible': 'Total Deductible',
  'dashboard.recentTx': 'Recent Transactions',
  'dashboard.pending': 'Pending',
  'dashboard.classified': 'Classified',
  'dashboard.monthlyExpenses': 'Monthly Expenses',
  'dashboard.byCategory': 'Expenses by Category',
  'dashboard.noBusinesses': 'No businesses yet',
  'dashboard.addFirst': 'Add your first business to get started',
  'dashboard.ytd': 'Year to date',
  'dashboard.viewAll': 'View all',
  'dashboard.quickActions': 'Quick actions',
  'dashboard.reviewPending': 'Review {n} pending →',
  'dashboard.vsPrevPeriod': 'vs. previous period',
  'tx.ruleSuggestTitle': 'Create an automatic rule?',
  'tx.ruleSuggestBody': 'You\'ve already classified {count} transactions from "{pattern}" as {category}. Create a rule so it classifies itself next time.',
  'tx.ruleSuggestConfirm': 'Create rule',
  'tx.ruleSuggestDismiss': 'Not now',

  // Nav - Recurring
  'nav.recurring': 'Recurring',

  // Recurring transactions
  'recurring.title': 'Recurring Income & Expenses',
  'recurring.subtitle': 'Create a template once and it keeps recording itself — rent, payroll, subscriptions.',
  'recurring.add': 'New recurring',
  'recurring.descriptionLabel': 'Description',
  'recurring.descriptionPlaceholder': 'Store rent, Payroll, Netflix...',
  'recurring.amountLabel': 'Amount',
  'recurring.frequencyLabel': 'Frequency',
  'recurring.startDateLabel': 'First date',
  'recurring.endDateLabel': 'End date (optional)',
  'recurring.endDateHint': 'Leave empty for it to continue indefinitely',
  'recurring.added': 'Recurring template created!',
  'recurring.list': 'Active templates',
  'recurring.noTemplates': 'You have no recurring income or expenses',
  'recurring.noTemplatesHint': 'Create one to stop logging the same entry every month',
  'recurring.next': 'next:',
  'recurring.active': 'Active',
  'recurring.paused': 'Paused',
  'recurring.pause': 'Pause',
  'recurring.resume': 'Resume',
  'dashboard.noPending': 'No pending transactions',

  // Transactions
  'tx.import': 'Import transactions',
  'tx.viewAll': 'View all',
  'tx.pending': 'Pending',
  'tx.classified': 'Classified',
  'tx.needsReview': 'Review',
  'tx.selectPending': 'Select pending',
  'tx.aiClassify': 'Classify with AI',
  'tx.split': 'Split',
  'tx.date': 'Date',
  'tx.description': 'Description',
  'tx.amount': 'Amount',
  'tx.category': 'Category',
  'tx.status': 'Status',
  'tx.deductible': 'Deductible',
  'tx.unassigned': '— unassigned —',
  'tx.noData': 'No transactions',
  'tx.prev': 'Previous',
  'tx.next': 'Next',
  'tx.aiSuccess': 'AI classified {auto} automatically, {review} need review.',
  'tx.splitTitle': 'Split Transaction',
  'tx.addRow': '+ Row',
  'tx.saveSplit': 'Save split',

  // Business
  'business.name': 'Business name',
  'business.industry': 'Industry',
  'business.entity': 'Entity type',
  'business.taxYear': 'Tax year',
  'business.create': 'Create business',
  'business.switch': 'Switch business',
  'business.addNew': '+ Add business',
  'business.select': 'Select a business',
  'business.noBusinesses': 'No businesses',
  'business.setActive': 'Set active',

  // Admin
  'admin.title': 'Administration Panel',
  'admin.totalAccounts': 'Total accounts',
  'admin.totalAccountants': 'Accountants',
  'admin.totalTeamMembers': 'Team members',
  'admin.totalBusinesses': 'Businesses',
  'admin.totalTx': 'Transactions',
  'admin.aiUsage': 'AI Classifications',
  'admin.suspend': 'Suspend',
  'admin.activate': 'Activate',
  'admin.createAccount': 'Create account',
  'admin.editAccount': 'Edit account',
  'admin.plan': 'Plan',
  'admin.status': 'Status',
  'admin.active': 'Active',
  'admin.suspended': 'Suspended',
  'admin.lastLogin': 'Last login',
  'admin.businesses': 'Businesses',
  'admin.never': 'Never',
  'admin.allTypes': 'All types',
  'admin.allPlans': 'All plans',
  'admin.allStatus': 'All statuses',

  // Plan
  'plan.none': 'No plan (blocked)',
  'plan.trial': 'Free trial',
  'plan.basic': 'Basic',
  'plan.plus': 'Plus',
  'plan.enterprise': 'Enterprise',
  'plan.custom': 'Custom',

  // Role
  'role.superadmin': 'Super Admin',
  'role.accountant': 'Accountant',
  'role.team_member': 'Team member',

  // Common
  'common.save': 'Save',
  'common.cancel': 'Cancel',
  'common.delete': 'Delete',
  'common.edit': 'Edit',
  'common.create': 'Create',
  'common.loading': 'Loading...',
  'common.error': 'Error',
  'common.success': 'Success',
  'common.close': 'Close',
  'common.next': 'Next',
  'common.back': 'Back',
  'common.search': 'Search',
  'common.filter': 'Filter',
  'common.all': 'All',
  'common.noData': 'No data available',
  'common.confirm': 'Confirm?',
  'common.yes': 'Yes',
  'common.no': 'No',
  'common.clear': 'Clear',
  'common.export': 'Export',
  'common.import': 'Import',
  'common.required': 'Required',
  'common.optional': 'Optional',

  // Reports
  'reports.title': 'Reports',
  'reports.from': 'From',
  'reports.to': 'To',
  'reports.thisYear': 'This year',
  'reports.pnl': 'P&L',
  'reports.byCategory': 'By Category',
  'reports.monthly': 'Monthly',
  'reports.exportPDF': 'Export PDF',
  'reports.exportExcel': 'Export Excel',
  'reports.noExpenses': 'No classified expenses in this period',

  // Import
  'import.title': 'Import Transactions',
  'import.upload': 'Upload file',
  'import.map': 'Map columns',
  'import.result': 'Result',
  'import.dragDrop': 'Drag your bank statement here or click to browse',
  'import.formats': 'Supported formats: CSV, XLSX, PDF',
  'import.chooseFile': 'Choose File',
  'import.bankName': 'Bank name (to save this mapping)',
  'import.savedFormats': 'Saved formats',
  'import.mapColumns': 'Map your columns to the fields:',
  'import.required': 'Required',
  'import.notMapped': '— not mapped —',
  'import.back': 'Back',
  'import.importBtn': 'Import Transactions',
  'import.importing': 'Importing...',
  'import.complete': 'Import Complete',
  'import.imported': 'Imported',
  'import.duplicates': 'Duplicates skipped',
  'import.total': 'Total rows',
  'import.errors': 'Errors',
  'import.importAnother': 'Import another',
  'import.reviewTx': 'Review Transactions',

  // Rules
  'rules.title': 'Classification Rules',
  'rules.add': 'Add Rule',
  'rules.pattern': 'If {campo} contains',
  'rules.field': 'Field to evaluate',
  'rules.category': 'Assign category',
  'rules.priority': 'Priority (higher = first)',
  'rules.noRules': 'No rules. Add your first rule above.',
  'rules.info': 'Rules automatically classify future transactions. Higher priority = applied first.',

  // Settings
  'settings.title': 'Settings',
  'settings.yourBusinesses': 'Your Businesses',
  'settings.addBusiness': 'Add Business',
  'settings.profile': 'Profile',
  'settings.plan': 'My Plan',

  // Common extras
  'common.actions': 'Actions',
  'common.allStatus': 'All statuses',
  'common.allCategories': 'All categories',
  'common.del': 'Delete',
  'common.yes100': 'Yes (100%)',
  'common.fifty': '50%',
  'common.notSet': '— not set —',
  'common.select': '— select —',
  'common.transactions': 'transactions',

  // Transactions extras
  'tx.aiClassifyCount': 'AI Classify ({n})',
  'tx.allStatus': 'All statuses',
  'tx.allCategories': 'All categories',
  'tx.classifying': 'Classifying...',
  'tx.delConfirm': 'Delete this transaction?',
  'tx.delBulkConfirm': 'Delete {n} transactions? This cannot be undone.',
  'tx.delBulkSuccess': '{n} transactions deleted',
  'tx.delBulkPartial': '{ok} deleted, {fail} failed',
  'tx.noDataFilters': 'Try adjusting the filters',
  'tx.splitParts': 'Split ({n} parts)',
  'tx.aiSuggestion': 'AI: {cat} ({conf})',
  'tx.total': '{total} transactions',

  // Import extras
  'import.uploadTitle': 'Upload Bank Statement',
  'import.formatsError': 'Only CSV, XLSX and PDF files supported',
  'import.mapRequired': 'Please map: date, description, and amount (or debit+credit)',
  'import.importFailed': 'Import failed',
  'import.required_fields': '* Required. Map "amount" OR both "debit" and "credit".',
  'import.fieldDate': 'date',
  'import.fieldDesc': 'description',
  'import.fieldAmount': 'amount',
  'import.fieldDebit': 'debit',
  'import.fieldCredit': 'credit',

  // Reports extras
  'reports.expenseReport': 'Expense Report',
  'reports.period': 'Period',
  'reports.summary': 'Summary',
  'reports.totalIncome': 'Total Income',
  'reports.totalExpenses': 'Total Expenses',
  'reports.netProfit': 'Net Profit',
  'reports.totalDeductible': 'Total Deductible',
  'reports.expensesByCategory': 'Expenses by Category',
  'reports.monthlyBreakdown': 'Monthly Breakdown',
  'reports.month': 'Month',
  'reports.total': 'Total',
  'reports.count': 'Transactions',
  'reports.irsCode': 'Tax Code',
  'reports.deductible': 'Deductible',
  'reports.generating': 'Generating...',
  'reports.noData': 'No data for this period',
  'reports.corporateReport': 'Corporate Financial Report',
  'reports.confidential': 'Confidential',
  'reports.generatedOn': 'Generated on',
  'reports.incomeStatement': 'Income Statement',
  'reports.concept': 'Concept',
  'reports.pendingClassify': 'Pending classification',
  'reports.classifiedCount': 'Classified',
  'reports.monthlyTrend': 'Monthly Trend',
  'reports.net': 'Net',
  'reports.page': 'Page',
  'reports.of': 'of',
  'reports.income': 'Income',
  'reports.expenses': 'Expenses',
  'reports.subtotal': 'Subtotal',
  'reports.txByCategory': 'Transactions by Category',
  'reports.date': 'Date',
  'reports.description': 'Description',
  'reports.type': 'Type',
  'reports.status': 'Status',
  'reports.inflow': 'Income',
  'reports.outflow': 'Expense',
  'reports.statusClassified': 'Classified',
  'reports.statusPending': 'Pending',
  'reports.statusReview': 'Review',

  // Business extras
  'business.created': 'Business created!',
  'business.failed': 'Creation failed',
  'business.taxYearLabel': 'Tax Year',

  // Nav extras
  'nav.banks': 'Banks',
  'nav.businesses': 'Businesses',
  'nav.categories': 'Categories',
  'nav.receipts': 'Receipts',

  // Receipts
  'receipts.title': 'Receipts',
  'receipts.scan': 'Scan Receipt',
  'receipts.history': 'History',
  'receipts.dropzone': 'Drop your receipts here or click to browse',
  'receipts.formats': 'Supports: JPG, PNG, PDF, WEBP, HEIC',
  'receipts.choose': 'Choose Files',
  'receipts.scanning': 'Scanning...',
  'receipts.reviewing': 'Pending review',
  'receipts.processed': 'Processed this session',
  'receipts.noReceipts': 'No receipts scanned yet. Upload a photo of a receipt to get started.',
  'receipts.noHistory': 'No receipts found for this business.',
  'receipts.totalReceipts': 'receipts',
  'receipts.reviewLabel': 'Review',
  'receipts.merchant': 'Merchant / Store',
  'receipts.merchantPlaceholder': 'Store name',
  'receipts.deductibility': 'Deductibility',
  'receipts.confirm': 'Confirm',
  'receipts.reject': 'Discard',
  'receipts.confirmed': 'Confirmed!',
  'receipts.rejected': 'Discarded',
  'receipts.aiExtracted': 'AI extracted items',
  'receipts.selectBusiness': 'Select a business first',
  'receipts.invalidFormat': 'Invalid format. Use JPG, PNG, PDF, WEBP or HEIC.',
  'receipts.tooLarge': 'File exceeds the 10 MB limit.',
  'receipts.rejectConfirm': 'Discard this receipt? The associated transaction will be deleted.',

  // Banks
  'banks.title': 'Banks',
  'banks.savedMappings': 'Saved mappings',
  'banks.noMappings': 'No banks configured',
  'banks.addBank': 'Add bank',
  'banks.importHistory': 'Import history',
  'banks.noHistory': 'No imports recorded',
  'banks.columns': 'Mapped columns',

  // Businesses management
  'biz.title': 'Businesses',
  'biz.allBusinesses': 'All businesses',
  'biz.noBusinesses': 'No businesses registered',
  'biz.owner': 'Owner',
  'biz.edit': 'Edit',
  'biz.transactions': 'transactions',
  'biz.activated': 'Active business: {name}',
  'biz.updated': 'Business updated',
  'biz.saveFailed': 'Save failed',
  'biz.deleteFailed': 'Delete failed',
  'biz.deleted': '"{name}" deleted',

  // Categories
  'cat.title': 'Categories',
  'cat.add': 'Add Category',
  'cat.name': 'Name',
  'cat.irsCode': 'Tax Code',
  'cat.description': 'Description',
  'cat.system': 'System',
  'cat.custom': 'Custom',
  'cat.noCategories': 'No categories',
  'cat.added': 'Category added!',
  'cat.deleted': 'Category deleted',
  'cat.deleteConfirm': 'Delete this category?',

  // Settings extras
  'settings.saveProfile': 'Save Profile',
  'settings.emailReadOnly': 'Email cannot be changed',
  'settings.memberSince': 'Member since',
  'settings.password': 'Password',
  'settings.invitedUsers': 'Invited users',
  'settings.inviteUser': 'Invite user',
  'settings.currentPassword': 'Current password',
  'settings.newPassword': 'New password',
  'settings.changePassword': 'Change password',
  'settings.profileSaved': 'Profile saved!',
  'settings.passwordChanged': 'Password updated!',
  'settings.planInfo': 'Your current plan',
  'settings.noInvited': 'No invited users',
  'settings.accessLevel': 'Access level',
  'settings.billing': 'Billing',
  'settings.aiUsageThisMonth': 'AI-classified transactions this month',
  'settings.aiUsageUnlimited': '{n} classified with AI this month — no monthly limit set',
  'settings.trialActive': "You're on your free trial — {days} day(s) left. Pick a plan below to keep access once it ends.",
  'settings.upgradeTo': 'Upgrade to {plan}',
  'settings.changeTo': 'Change to {plan}',
  'settings.cancelSubscription': 'Cancel subscription',
  'settings.resumeSubscription': 'Resume subscription',
  'settings.ownerOnlyBilling': 'Only the account owner can manage billing',
  'settings.alreadyTopPlan': "You already have the highest plan available for purchase. Contact support if you need something different.",
  'settings.blockedOwner': "Your account doesn't have an active plan. Choose a plan below to activate access.",
  'settings.blockedMember': "Your account doesn't have an active plan. Contact the account owner to restore access.",
  'settings.subscriptionCanceled': 'Your subscription ended and you lost access — pick a plan below to reactivate it',
  'settings.subscriptionPaymentFailed': 'The last charge failed — update your payment method to avoid interruptions',
  'settings.confirmCancel': 'Cancel your subscription? You will keep access until the end of the period you already paid for.',
  'settings.checkoutComplete': 'Payment received — your plan will activate in a few seconds',

  // Rules extras
  'rules.learned': 'AI-Learned Rules',
  'rules.learnedDesc': 'Patterns detected from your manual classifications',
  'rules.noLearned': 'No learned patterns yet',
  'rules.learnedHint': 'Classify transactions manually and AI will learn the patterns',
  'rules.disable': 'Disable',
  'rules.enable': 'Enable',
  'rules.appliedTo': 'Applied to {n} transactions',
  'rules.confirmRule': 'Confirm as rule',
  'rules.manualTitle': 'Manual Rules',
  'rules.manualDesc': 'Auto-classify future transactions by keyword',
  'rules.basicPlan': 'Basic Plan — Manual keyword rules',
  'rules.plusPlan': 'Plus/Enterprise Plan — AI learns from your corrections',
  'rules.adding': 'Adding...',
  'rules.added': 'Rule added!',
  'rules.delete': 'Delete',
  'rules.activeRules': 'Active Rules',
  'rules.noActiveRules': 'No active rules',
  'rules.patternLabel': 'If description contains',
  'rules.upgrade': 'Upgrade to Plus for automatic AI classification',

  // Audit log
  'audit.title': 'Audit Log',
  'audit.events': 'events',
  'audit.noEvents': 'No events recorded yet',
  'audit.clearBtn': 'Clear audit log',
  'audit.clearConfirm': 'Delete {total} events?',
  'audit.clearing': 'Deleting...',

  // Reports CSV
  'reports.exportCSV': 'Export CSV',
  'reports.corporateBtn': 'Corporate Report',
  'reports.txByCategoryBtn': 'Tx by Category',
  'reports.txByCategoryPdfBtn': 'Transactions PDF',
  'reports.availableInPlus': 'Available in Plus and Enterprise plans',
  'reports.uncategorized': 'Uncategorized',

  // Dashboard extras
  'dashboard.allMovements': 'All transactions',

  // Transactions extras II
  'tx.searchPlaceholder': 'Search by description...',
  'tx.selected': '{n} selected',
  'tx.classifyAs': 'Classify as...',
  'tx.apply': 'Apply',
  'tx.deleteCount': 'Delete ({n})',
  'tx.xOf': '{x} of {total}',
  'tx.loadingMore': 'Loading more...',
  'tx.endOfList': '— End of transactions —',
  'tx.deleted': 'Transaction deleted',
  'tx.nClassified': '{n} transactions classified',
  'tx.nClassifyErrors': '{n} errors classifying',
  'tx.aiDone': 'Classification complete',
  'tx.aiError': 'Error classifying with AI',
  'tx.splitCatRequired': 'Select at least one category',
  'tx.splitTotalMismatch': 'Split amounts must add up to the transaction total',
  'tx.addTransaction': 'Add transaction',
  'tx.generating': 'Generating...',
  'tx.showingImportedBatch': 'Showing only the recently imported transactions ({total})',
  'tx.allTypes': 'Income and Expenses',
  'tx.creditsOnly': 'Income only',
  'tx.debitsOnly': 'Expenses only',
  'tx.addManualTitle': 'Add manual transaction',
  'tx.type': 'Type',
  'tx.typeDebit': 'Expense (debit)',
  'tx.typeCredit': 'Income (credit)',
  'tx.descriptionPlaceholder': 'E.g. Office rent payment',
  'tx.categoryOptional': 'Category (optional)',
  'tx.deductibleOptional': 'Deductible (optional)',
  'tx.notesOptional': 'Notes (optional)',
  'tx.notesPlaceholder': 'Additional notes',
  'tx.recurringLabel': 'This is a recurring expense/income — repeat it multiple times',
  'tx.frequency': 'Frequency',
  'tx.weekly': 'Weekly',
  'tx.biweekly': 'Biweekly',
  'tx.monthly': 'Monthly',
  'tx.everyWeek': 'week',
  'tx.everyTwoWeeks': 'two weeks',
  'tx.everyMonth': 'month',
  'tx.repeatCountLabel': 'Number of repeats',
  'tx.recurringPreview': '{n} identical transactions will be created, starting on the date above, every {freq}.',
  'tx.saveTransaction': 'Save transaction',
  'tx.transactionAdded': 'Transaction added',
  'tx.transactionsAdded': '{n} transactions added',
  'tx.addRequiredFields': 'Date, description, and amount are required',
  'tx.addAmountPositive': 'The amount must be a positive number',
  'tx.addRepeatMin': 'Enter how many times it repeats (minimum 2)',
  'tx.addFailed': 'Could not create the transaction',
  'common.connectionError': 'Connection error',
  'common.saving': 'Saving...',

  // Common extras II
  'common.operationError': 'Error in operation',

  // Clasificar con IA
  'clasificar.subtitle': 'Upload your bank statement and AI classifies it automatically',
  'clasificar.formatsError': 'Only CSV, XLSX, or XLS files are accepted.',
  'clasificar.fieldDate': 'Date *',
  'clasificar.fieldDescription': 'Description *',
  'clasificar.fieldAmount': 'Amount',
  'clasificar.fieldDebit': 'Debit',
  'clasificar.fieldCredit': 'Credit',
  'clasificar.mapRequired': 'You must map at least: Date, Description, and Amount (or Debit/Credit).',
  'clasificar.readingFile': 'Reading file...',
  'clasificar.noValidTx': 'No valid transactions were found in the file.',
  'clasificar.classifyingCount': 'Classifying {n} transactions with AI...',
  'clasificar.classifyError': 'Error classifying with AI',
  'clasificar.saveError': 'Error saving transactions',
  'clasificar.savedCount': '{n} transactions saved',
  'clasificar.duplicatesSkippedCount': '{n} duplicates skipped',
  'clasificar.totalExpensesAmount': '{amount} in total expenses',
  'clasificar.aiConfidenceHeader': 'AI Confidence',
  'clasificar.summarySheet': 'Summary',
  'clasificar.reportHeading': 'Report — {name}',
  'clasificar.defaultBusinessName': 'Business',
  'clasificar.value': 'Value',
  'clasificar.downloadComplete': 'Download complete',
  'clasificar.downloadError': 'Error downloading report',
  'clasificar.pdfTitle': '{name} — Classification Report',
  'clasificar.summaryGeneral': 'General Summary',
  'clasificar.txCountLabel': 'No. of Transactions',
  'clasificar.planRequiredTitle': 'This feature is available on the Plus plan or higher',
  'clasificar.planRequiredDesc': 'Automatic AI classification requires a Plus, Enterprise, or Custom plan. Your current plan is',
  'clasificar.contactAdmin': 'Contact your administrator to upgrade your plan.',
  'clasificar.thisMonth': 'This month',
  'clasificar.classifiedThisMonth': '{n} classified this month',
  'clasificar.stepUpload': 'Upload',
  'clasificar.stepMap': 'Map',
  'clasificar.stepClassifying': 'Classifying',
  'clasificar.stepReview': 'Review',
  'clasificar.savedFormatsLabel': 'SAVED FORMATS',
  'clasificar.dropzoneTitle': 'Upload your bank statement',
  'clasificar.dropzoneHint': 'Drag the file here or use the button',
  'clasificar.chooseFile': 'Choose file',
  'clasificar.previewTitle': 'Preview — {name}',
  'clasificar.sampleRows': '{n} sample rows',
  'clasificar.columnMapping': 'Column mapping',
  'clasificar.bankNameLabel': 'Bank name (optional)',
  'clasificar.bankNamePlaceholder': 'Chase, Bank of America, Wells Fargo...',
  'clasificar.bankNameHint': "If you save it, we'll use it automatically next time.",
  'clasificar.whichColumn': 'Which column corresponds to each field?',
  'clasificar.notUsed': '— Do not use —',
  'clasificar.backArrow': '← Back',
  'clasificar.importAndClassify': 'Import and Classify with AI',
  'clasificar.processingHint': 'This may take a few seconds...',
  'clasificar.autoClassified': 'Auto-classified',
  'clasificar.toReview': 'To review',
  'clasificar.txImportedSuffix': 'transactions imported',
  'clasificar.inThisReviewSuffix': 'in this review',
  'clasificar.confidenceWarningPre': 'Rows in',
  'clasificar.yellow': 'yellow',
  'clasificar.confidenceWarningPost': 'have MEDIUM or LOW confidence — review and adjust the category if needed.',
  'clasificar.confidence': 'Confidence',
  'clasificar.notSavedPre': 'The transactions',
  'clasificar.notSavedBold': 'have not been saved yet',
  'clasificar.notSavedPost': 'Review the categories and click "Save to transactions" when you\'re ready.',
  'clasificar.saveCount': 'Save {n} transactions',
  'clasificar.anotherFile': 'Another file',
  'clasificar.classificationComplete': 'Classification complete!',
  'clasificar.downloadPDFBtn': 'Download PDF',
  'clasificar.downloadExcelBtn': 'Download Excel',
  'clasificar.viewInTransactions': 'View in Transactions',
  'clasificar.classifyAnother': 'Classify another file',

  // Asignaciones
  'asignaciones.title': 'Assignments',
  'asignaciones.subtitle': "Assign and track your team's accounting tasks.",
  'asignaciones.new': '+ New assignment',
  'asignaciones.selectBusiness': 'Select a business to see its assignments.',
  'asignaciones.statusPending': 'Pending',
  'asignaciones.statusInProgress': 'In progress',
  'asignaciones.statusCompleted': 'Completed',
  'asignaciones.statusCancelled': 'Cancelled',
  'asignaciones.filterAll': 'All',
  'asignaciones.loading': 'Loading...',
  'asignaciones.empty': 'No assignments',
  'asignaciones.emptyViewer': 'You have no pending assignments.',
  'asignaciones.emptyEditor': 'Create an assignment to start delegating work.',
  'asignaciones.overdue': '⚠ Overdue',
  'asignaciones.byPrefix': 'By',
  'asignaciones.unassigned': 'Unassigned',
  'asignaciones.notesToggle': '💬 Notes',
  'asignaciones.notesTooltip': 'View / add notes',
  'asignaciones.edit': 'Edit',
  'asignaciones.noNotesYet': 'No notes yet.',
  'asignaciones.addNotePlaceholder': 'Add a note or comment...',
  'asignaciones.addNoteBtn': 'Add',
  'asignaciones.noteSaveError': 'Error saving note',
  'asignaciones.deleteNoteConfirm': 'Delete this note?',
  'asignaciones.ctrlEnterHint': 'Ctrl+Enter to send',
  'asignaciones.editModalTitle': 'Edit assignment',
  'asignaciones.newModalTitle': 'New assignment',
  'asignaciones.titleLabel': 'Title *',
  'asignaciones.titlePlaceholder': 'Assignment title',
  'asignaciones.descriptionLabel': 'Description',
  'asignaciones.descriptionPlaceholder': 'Task details...',
  'asignaciones.assignToLabel': 'Assign to',
  'asignaciones.myself': 'Myself',
  'asignaciones.dueDateLabel': 'Due date',
  'asignaciones.statusLabel': 'Status',
  'asignaciones.titleRequired': 'Title is required',
  'asignaciones.saveError': 'Error saving',
  'asignaciones.deleteConfirm': 'Delete this assignment?',
  'asignaciones.notifyEmail': "📧 A notification email will be sent to the assigned user.",
  'asignaciones.saving': 'Saving...',
  'asignaciones.saveChanges': 'Save changes',
  'asignaciones.createBtn': 'Create assignment',

  // Documentos
  'documentos.title': 'Documents',
  'documentos.subtitle': 'Manage the business documents: {examples} and more.',
  'documentos.uploadBtn': '+ Upload document',
  'documentos.tabFiles': 'Uploaded files',
  'documentos.tabTypes': 'Document types',
  'documentos.selectBusiness': 'Select a business to see its documents.',
  'documentos.newType': '+ New type',
  'documentos.noTypes': 'No document types',
  'documentos.noTypesHint': 'Create types like {examples}, etc.',
  'documentos.createFirstType': '+ Create first type',
  'documentos.colFiles': 'Files',
  'documentos.allCount': 'All ({n})',
  'documentos.noDocs': 'No documents',
  'documentos.noDocsHint': 'Upload the first document with the button above.',
  'documentos.colFile': 'File',
  'documentos.colUploadedBy': 'Uploaded by',
  'documentos.download': 'Download',
  'documentos.editTypeTitle': 'Edit type',
  'documentos.newTypeTitle': 'New document type',
  'documentos.nameLabel': 'Name *',
  'documentos.namePlaceholder': 'Document type name',
  'documentos.descriptionOptionalLabel': 'Description (optional)',
  'documentos.requiredCheckbox': 'Required document',
  'documentos.uploadModalTitle': 'Upload document',
  'documentos.needTypeFirst': 'First create at least one document type.',
  'documentos.createTypeBtn': 'Create document type',
  'documentos.typeLabel': 'Document type *',
  'documentos.selectPlaceholder': 'Select...',
  'documentos.fileLabel': 'File *',
  'documentos.fileHint': 'PDF, images, Word, Excel — max. 10 MB',
  'documentos.notesOptionalLabel': 'Notes (optional)',
  'documentos.notesPlaceholder': 'Notes',
  'documentos.uploading': 'Uploading...',
  'documentos.uploadSubmitBtn': 'Upload',
  'documentos.nameRequired': 'Name is required',
  'documentos.saveTypeError': 'Save failed',
  'documentos.deleteTypeConfirmWithDocs': 'This type has {n} document(s). Delete anyway? The documents will also be deleted.',
  'documentos.deleteTypeConfirm': 'Delete the type "{name}"?',
  'documentos.selectFile': 'Select a file',
  'documentos.selectDocType': 'Select the document type',
  'documentos.uploadError': 'Upload failed',
  'documentos.deleteDocConfirm': 'Delete this document?',

  // Team (Usuarios)
  'team.title': 'Team Members',
  'team.subtitle': "Create access for your staff. Each user has their own login and sees the same businesses you do.",
  'team.addUser': '+ Add user',
  'team.noMembers': 'You have no team members',
  'team.noMembersHint': 'Add members so they can access the platform with their own username and password.',
  'team.addFirstUser': '+ Add first user',
  'team.colEmail': 'Email',
  'team.lastAccess': 'Last access',
  'team.superadmin': 'Superadmin',
  'team.inactive': 'Inactive',
  'team.addModalTitle': 'Add user to team',
  'team.inviteInfo': "We'll send them an email with a link to choose their own password. They will see the same businesses you do.",
  'team.creating': 'Creating...',
  'team.createUser': 'Create user',
  'team.userCreatedTitle': 'User created',
  'team.inviteSentText': "We sent an invitation email. If it doesn't arrive, share this link directly (valid for 7 days, single use):",
  'team.copied': 'Copied',
  'team.copy': 'Copy',
  'team.done': 'Done',
  'team.nameEmailRequired': 'Name and email are required',
  'team.createError': 'Error creating user',
  'team.passwordRequirements': 'Min 8 characters, one uppercase letter and one number',
  'team.deleteConfirm': 'Delete user "{name}"? This action cannot be undone.',
} as const

export type TranslationKey = keyof typeof en
export default en
