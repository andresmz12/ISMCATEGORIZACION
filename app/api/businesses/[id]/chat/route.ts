...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const budgetResult = await withAiBudget(businessId, async () => {
    ...
    // Record whatever tokens were actually spent even if a later iteration
    // failed — a partial multi-step exchange still cost real money. This must
    // never throw past this point: withAiBudget records inputTokens/outputTokens
    // from whatever this returns, so an uncaught throw here would lose the
    // record of tokens already spent in earlier iterations.
    try {
      ...
      console.error('Chat AI loop error:', e)
      errorResponse = NextResponse.json({ error: 'El asistente no pudo responder, intenta de nuevo' }, { status: 502 })
    }

    return {
      result: { finalText, errorResponse, messages },
  ...
  const { finalText, errorResponse, messages: finalMessages } = budgetResult.result
  if (errorResponse) return errorResponse

  return NextResponse.json({
    reply: finalText,
    history: [...finalMessages.filter(m => typeof m.content === 'string'), { role: 'assistant', content: finalText }].slice(-MAX_HISTORY_MESSAGES),
  })
}
